...
import com.example.MovieCatalog.domain.use_case.get_movie_detail.GetMovieDetailsUseCase
...
@HiltViewModel
class MovieDetailViewModel @Inject constructor(private val getMovieDetailsUseCase: GetMovieDetailsUseCase,private val stateHandle : SavedStateHandle): ViewModel(){
    ...
    private fun getMovieDetail(imdbId : String){
        getMovieDetailsUseCase.executeGetMovieDetails(imdbId =imdbId).onEach {
            when (it) {
                ...
                is Resource.Error -> {
                    _state.value = MovieDetailState (error = "Hata Dostum: Failed to getMovie")
                }
                is Resource.Loading -> {
